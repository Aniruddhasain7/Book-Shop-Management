...
def sell_book():
    ...
    __cur.execute("SELECT Book_Name, Available_Stock, Cost_per_book FROM stock WHERE Book_No = %s", (bno,))
    book_data = __cur.fetchone()

    if not book_data:
        print("\n Book Not Available")
        return
    
    bname, available_stock, db_cost = book_data

    if available_stock <= 0:
        print("\n Book is OUT OF STOCK")
        return

    print(f"Book Found: {bname}")
    ...
    print(f"Cost            : {cost}")
    print(f"Date of Purchase: {__dt.date.today()}")
    print("=" * 40)
